...
def sam_loss(x, y):
    ...
    den = torch.sqrt(torch.multiply(torch.sum(x**2+1e-6, 1), torch.sum(y**2+1e-6, 1)))
...
def HyperspectralSWTLoss(x, y, wavelet,loss_weights=None, reduction='mean'):
    ...
    for band in range(x.shape[1]):
        sr_band = x[:, band:band+1, :, :]
        hr_band = y[:, band:band+1, :, :]
        ...
        loss_subband_LL = loss_weights[0] * criterion(LL_sr, LL_hr)
        loss_subband_LH = loss_weights[1] * criterion(LH_sr, LH_hr)
        loss_subband_HL = loss_weights[2] * criterion(HL_sr, HL_hr)
        loss_subband_HH = loss_weights[3] * criterion(HH_sr, HH_hr)
